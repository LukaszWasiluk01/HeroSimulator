    ...
    public class GameService
    {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                switch (itemType)
                {
                    ...
                    case 4:
                        ...
                        break;
                    default:
                        item = new Ring($"Pierscien poziomu {_hero.Level}", rarity) { BonusStrength = statBonus / 3, BonusDexterity = statBonus / 3, BonusIntelligence = statBonus / 3, Price = price };
                        break;
                }
        ...
        public void EquipItem(Item item)
        {
            if (_hero.Backpack.Contains(item))
            {
                ...
                {
                    if (_hero.EquippedWeapon != null) _hero.Backpack.Add(_hero.EquippedWeapon);
                    _hero.EquippedWeapon = weapon;
                }
                else if (item is Armor armor)
                {
                    if (_hero.EquippedArmor != null) _hero.Backpack.Add(_hero.EquippedArmor);
                    _hero.EquippedArmor = armor;
                }
                else if (item is Pants pants)
                {
                    if (_hero.EquippedPants != null) _hero.Backpack.Add(_hero.EquippedPants);
                    _hero.EquippedPants = pants;
                }
                else if (item is Boots boots)
                {
                    if (_hero.EquippedBoots != null) _hero.Backpack.Add(_hero.EquippedBoots);
                    _hero.EquippedBoots = boots;
                }
                else if (item is Amulet amulet)
                {
                    if (_hero.EquippedAmulet != null) _hero.Backpack.Add(_hero.EquippedAmulet);
                    _hero.EquippedAmulet = amulet;
                }
                else if (item is Ring ring)
                {
                    if (_hero.EquippedRing != null) _hero.Backpack.Add(_hero.EquippedRing);
        ...
        private void LevelUp()
        {
            ...
            OnLogMessage?.Invoke($"Awans na {_hero.Level} poziom! Odnowiono zdrowie.");
        }
    }
}